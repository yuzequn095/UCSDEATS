...
public class NewUserActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mProfileButton = (ImageButton) findViewById(R.id.upload_profile);
        mProfileButton.setImageDrawable(getResources().getDrawable(R.drawable.ucsd_eats_logo));
        ...
        mMajorView.setOnEditorActionListener(new TextView.OnEditorActionListener() {
            @Override
            public boolean onEditorAction(TextView textView, int id, KeyEvent keyEvent) {
                return true;
            }
        ...
        Log.d(TAG, "EnterActivity:success");
    }
    ...
    private boolean mayRequestContacts() {
        ...
        if (shouldShowRequestPermissionRationale(READ_EXTERNAL_STORAGE)) {
            Snackbar.make(mProfileButton, R.string.permission_rationale, Snackbar.LENGTH_INDEFINITE)
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            Log.d(TAG, "data validation:failure");
            focusView.requestFocus();
        } else {
            // Show a progress spinner, and kick off a background task to
            // perform the user login attempt.
            Log.d(TAG, "data validation:success");
            showProgress(true);
    ...
    private boolean isEmailValid(String email) {
        ...
        if (email.length() >5 &&email.contains("@") && email.contains(".") && email.charAt(email.length()-4) == '.'){
            return true;
        }
        else
            return false;
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
                    ...
                    .addOnCompleteListener(NewUserActivity.this, new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            ...
                            if (task.isSuccessful()) {
                                ...
                                NewUserActivity.this.userID = user.getUid();
                                //create user profile in DataBase

                                //First, add profile picture
                                //Convert to byte array
                                Drawable drawable = mProfileButton.getDrawable();
                                ...
                                //Upload to storage
                                profilePictureRef = storageRef.child(Constant.profileSTR).
                                        child(user.getUid() + Constant.jpgSTR);
                                ...
                                uploadTask.addOnFailureListener(new OnFailureListener() {
                                    ...
                                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                                        ...
                                        // as size, content-type, etc.
                                        // ...
                                        Log.d(TAG, "Profile picture upload: success.");
                                        profilePictureFilename = NewUserActivity.this.userID +
                                                Constant.jpgSTR;
                                ...
                                String gender = "";
                                String intro = "Introduce yourself";
                                if (mGenderGroup.getCheckedRadioButtonId() != -1) {
                                    ...
                                    RadioButton btn = (RadioButton) mGenderGroup.getChildAt
                                            (radioId);
                                    gender = (String) btn.getText();
                                }

                                Spinner foodS = (Spinner)findViewById(R.id.food_spinner);
                                food = foodS.getSelectedItem().toString();

                                String phoneNumber = mPhoneView.getText().toString();
                                User newUser = new User(mEmail, name, major, gender, phoneNumber,
                                        intro,food, NewUserActivity.this.profilePictureFilename);
                                newUser.addFriend(userID);
                                newUser.profilePicFilename = profilePictureFilename;
                                mDatabase.child(Constant.userSTR).child(userID).setValue(newUser);
        ...
        protected void endingFunction(final Boolean success){
            mAuthTask = null;
            showProgress(false);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode == RESULT_OK) {
            ...
            if(requestCode == 100) {
                ...
                mProfileButton.setImageURI(selectedImage);
            }
        }
    }
}
